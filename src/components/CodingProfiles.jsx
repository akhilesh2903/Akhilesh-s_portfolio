...
const PlatformIcon = ({ platform }) => {
  if (platform === 'GitHub') return (
    <svg viewBox="0 0 24 24" fill="currentColor" className="profile-platform-icon" aria-hidden="true">
...
const ProfileCard = ({ profile }) => {
  ...
  return (
    <a
      ...
      className="glass-heavy profile-card"
      style={{ '--platform-color': profile.color, transformStyle: 'preserve-3d' }}
      data-cursor="github"
    >
      {/* Glow */}
      <div className="profile-glow" style={{ background: `radial-gradient(circle, ${profile.color}15 0%, transparent 70%)` }} aria-hidden="true" />

      {/* Top */}
      <div className="profile-top">
        <div className="profile-icon-wrap" style={{ borderColor: `${profile.color}40`, background: `${profile.color}0d` }}>
          <PlatformIcon platform={profile.platform} />
        </div>
        <div className="profile-ext-icon" aria-hidden="true">↗</div>
      </div>

      {/* Info */}
      <div className="profile-name" style={{ color: profile.color }}>{profile.platform}</div>
      <div className="profile-handle mono">@{profile.handle}</div>
      <p className="profile-desc">{profile.description}</p>

      {/* Animated border */}
      <div className="profile-border-glow" style={{ background: `linear-gradient(90deg, transparent, ${profile.color}60, transparent)` }} aria-hidden="true" />
    </a>
  );
};
...
const CodingProfiles = () => {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.from('.profile-card', {
        scrollTrigger: { trigger: '.profiles-grid', start: 'top 80%' },
        opacity: 0,
        y: 60,
        stagger: 0.15,
        scale: 0.95,
  ...
  return (
    <section id="profiles" className="profiles" ref={sectionRef}>
      <div className="container">
        <div className="profiles-header">
          <div className="section-label">Online Presence</div>
          <h2 className="section-title">
            Code. <span className="gradient-text">Build.</span> Compete.
          </h2>
          <p className="profiles-sub">Find me across the platforms where I write, share, and collaborate.</p>
        </div>
